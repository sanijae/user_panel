...
export default function Topics() {
   ...
   const navigate = useNavigate()
    useEffect(()=>{
        return(async()=>{
            try {
                setLoading(true)
                const res = await getDocs(collection(db,'topics'))
    ...
    async function searchTitle(){
        try {
            const docRef = doc(db,'topics',`${searchData}`)
    ...
    async function searchDepartment(){
        try {
            const result = await getDocs(query(collection(db,'topics'),where('department','==',searchDept)))//,where('department','==',searchData)
            if (result){
  ...
  return (
    <Container> 
      <ListContainer>
        <HeroHeader>
            <HeroTitle style={{textAlign:'center'}}>Your final year complete reaserch projects topics is ready here for you to download</HeroTitle>
             <SearchContainer>
                <RowSearch>
                    <SearchInput type={'text'} value={searchData} onChange={(e)=>setSearchData(e.target.value.toLowerCase())}
                     placeholder='Search topics title here' />
                ...
                <RowSearch>
                    ...
                    placeholder='Search topics by your Department here' />
            ...
            Find your NCE, ND, HND, Undergraduates and Postgraduates ready made school projects topics here for free
            </HeroSubtitle>
        </HeroHeader>
       {loading ? <Loading>
        <Icon>
       ...
       :<>
        ...
        {searchResult ? <TitleContainer onClick={()=>navigate(`/TopicDetail/${searchResult.title}`)}>
        ...
        : resDept.length !== 0 ? resDept.map((dt,i)=>(
            <TitleContainer key={i} onClick={()=>navigate(`/TopicDetail/${dt.title}`)}>
        ...
        : data.map((dt,i)=>{
            return <TitleContainer key={i} onClick={()=>navigate(`/TopicDetail/${dt.title}`)}>
                <Title>
                    {dt.title}
                </Title>
                <AuthorContainer>
