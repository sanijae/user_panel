...
export default function Proposal() {
   ...
   const navigate = useNavigate()
    useEffect(()=>{
        return(async()=>{
            try {
                setLoading(true)
                const res = await getDocs(collection(db,'proposals'))
    ...
    async function searchTitle(){
        try {
            const docRef = doc(db,'proposals',`${searchData}`)
    ...
    async function searchDepartment(){
        try {
            const result = await getDocs(query(collection(db,'proposals'),where('department','==',searchDept)))//,where('department','==',searchData)
            if (result){
  ...
  return (
    <Container> 
      <ListContainer>
        <HeroHeader>
            <HeroTitle style={{textAlign:'center'}}>Your final year complete reaserch projects proposals is ready here for you to download</HeroTitle>
             <SearchContainer>
                <RowSearch>
                    <SearchInput type={'text'} value={searchData} onChange={(e)=>setSearchData(e.target.value.toLowerCase())}
                     placeholder='Search projects proposals title here' />
                ...
                <RowSearch>
                    ...
                    placeholder='Search projects proposals by your Department here' />
            ...
            Find your NCE, ND, HND, Undergraduates and Postgraduates ready made school projects proposals here for free
            </HeroSubtitle>
        </HeroHeader>
       {loading ? <Loading>
        <Icon>
       ...
       :<>
        ...
        {searchResult ? <TitleContainer onClick={()=>navigate(`/ProposalDetail/${searchResult.title}`)}>
        ...
        : resDept.length !== 0 ? resDept.map((dt,i)=>(
            <TitleContainer key={i} onClick={()=>navigate(`/ProposalDetail/${dt.title}`)}>
        ...
        : data.map((dt,i)=>{
            return <TitleContainer key={i} onClick={()=>navigate(`/ProposalDetail/${dt.title}`)}>
                <Title>
                    {dt.title}
                </Title>
                <AuthorContainer>
